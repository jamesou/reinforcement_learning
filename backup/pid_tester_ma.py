...
RL_DIR='rl/dir'
name = 'pid_ma_tune_experiment_name'
...
if __name__=='__main__':
    for k in range(n_iter):
        ...
        for person in person_grid:
            ...
            sample_time = 5
            # name='child#001'
            u2ss = p.query('Name=="{}"'.format(person)).u2ss.item()
            bw = p.query('Name=="{}"'.format(person)).BW.item()
            basal = u2ss * bw / 6000
            cr = q.query('Name=="{}"'.format(person)).CR.item()
            cf = q.query('Name=="{}"'.format(person)).CF.item()
