...
def train(kwargs):
    ...
    save_path = 'saves_rlkit_21d'  # where the outputs will be saved
    ...
    # finish = True  # Used to run more test seeds for a fully trained method
    finish = kwargs['finish']  # Used to run more test seeds for a fully trained method
    ...
    num_steps_per_epoch = 6048              # 使用21天的数据作为训练集
    # num_steps_per_eval = 2880               # 使用10天的数据作为验证集
    num_steps_per_eval = 4032               # 使用14天的数据作为验证集

    loss_function = nn.SmoothL1Loss
    # reward_fun = 'magni_bg'
    reward_fun = kwargs['reward_fun']
    snapshot_gap = 1
    discount = 0.99
    policy_lr = 3e-4
    qf_lr = 3e-4
    ...
    for setting in itertools.product(*option_dict.values()):
        ...
        variant = dict(
            ...
            model_type='sac',
            include_time=False,
            include_meal=kwargs['include_meal'],
            ...
            carb_error_std=0,
            carb_miss_prob=0,
            # bg_prediction=r'D:\MyWorks\糖尿病预测\强化学习方法\rl4bg\bgp\bg_prediction_models\SeqMo_all\all_data.pt',  # 是否有血糖预测模型
            bg_prediction=kwargs['bg_prediction'],  # 是否有血糖预测模型
            kwargs=kwargs
        )
        run_func = run_em_sac
        if finish:
    ...
    if __name__ == '__main__':
        if debug:
            ...
            for c in tuples:
                ...
                run_config, run_func=c
                # if run_config['patient_name'] in ['child#003','child#004','child#005','child#006','child#007']:
                if run_config['patient_name'] in kwargs['patient_list']:
                # if run_config['patient_name'] in ['child#006','adolescent#006']:
                # if run_config['patient_name'] in ['adult#006']:
                # if run_config['patient_name'] in ['child#006','adolescent#006','adult#006']:
                    print(run_config['patient_name'])
                    run_func(run_config)

            # run_manager.run_until_empty(300)
...
if __name__ == "__main__":
    # args = {
    #     'base_name': 'sac_bg_prediction_gmse',
    #     'finish': True,
    #     'reward_fun': 'magni_bg',
    #     # 'bg_prediction':r'D:\MyWorks\糖尿病预测\强化学习方法\rl4bg\bgp\bg_prediction_models\SeqMo_all\all_data.pt',  # 是否有血糖预测模型
    #     'bg_prediction':r'D:\MyWorks\糖尿病预测\强化学习方法\rl4bg\bgp\bg_prediction_models\gmse\all_data.pt',  # 是否有血糖预测模型
    #     'include_meal': False
    # }
    # train(args)
    #  reward_fun=['magni_reward_duration','magni_bg']

    # args = {
    #     # 'base_name': 'test',
    #     'base_name': 'sac_duration_meal',
    #     'finish': False,
    #     # 'reward_fun': 'magni_bg',
    #     'reward_fun': 'magni_reward_duration',
    #     # 'bg_prediction':r'D:\MyWorks\糖尿病预测\强化学习方法\rl4bg\bgp\bg_prediction_models\SeqMo_all\all_data.pt',  # 是否有血糖预测模型
    #     # 'bg_prediction': r'D:\MyWorks\糖尿病预测\强化学习方法\rl4bg\bgp\bg_prediction_models\gmse\all_data.pt',  # 是否有血糖预测模型
    #     'bg_prediction': None,  # 是否有血糖预测模型
    #     'include_meal': True,
    #     # 'patient_list': ['adolescent#006','adult#006']
    #     # 'patient_list': ['child#008','adolescent#008','adult#008','child#007','adolescent#007','adult#007']
    #     # 'patient_list': ['adult#007','adult#008']
    #     'patient_list': [ 'adult#009']
    #
    # }
    # train(args)
    ...
    args = {
        ...
        'include_meal': True,
        # 'patient_list': ['adolescent#006','adult#006']
        # 'patient_list': ['child#008','child#007','adolescent#007']
        'patient_list': ['adolescent#009', 'adult#009']
        # 'patient_list': ['adolescent#007', 'adult#007']
    }
    train(args)

    args = {
        # 'base_name': 'test',
        'base_name': 'sac_duration',
        'finish': False,
        'reward_fun': 'magni_reward_duration',
        # 'reward_fun': 'magni_reward_duration',
        # 'bg_prediction':r'D:\MyWorks\糖尿病预测\强化学习方法\rl4bg\bgp\bg_prediction_models\SeqMo_all\all_data.pt',  # 是否有血糖预测模型
        ...
        'include_meal': False,
        # 'patient_list': ['adolescent#006','adult#006']
        'patient_list': ['child#009', 'adolescent#009', 'adult#009']
        # 'patient_list': ['adolescent#007', 'adult#007']
    }
    train(args)
