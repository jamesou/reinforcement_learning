...
import torch
from bgp.rlkit.launchers.launcher_util import setup_logger,logger
...
def reward_name_to_function(reward_name):
    ...
    elif reward_name == 'risk_diff_duration':
        reward_fun = reward_functions.risk_diff_duration
...
def  get_best_itr(variant, max=None, seed=None, use_min=True):
    ...
    try:
        ...
        return -1
    progress=progress.dropna().reset_index(drop=True)
    progress=progress.iloc[:300,:].reset_index(drop=True)

    # print(progress)
    # 将可以转化的类型转成数字
    progress=progress.apply(pd.to_numeric,errors='ignore')
    # print("调试信息", progress.apply(pd.to_numeric,errors='ignore').dtypes['GLen'])
...
def simulate_policy(variant, itr=None, save_q=False, cpu_only=False, gpu_remap=None, new_eval=True,
    ...
    print('当前的编号为:'+str(sim_seed_submod))
    print(run_device)
    if variant['model_type'] == 'sac':
        ...
        env = bgp_env.DeepSACT1DEnv(reward_fun=reward_fun,
                                    ...
                                    bg_prediction=variant['bg_prediction'])
    else:
    ...
    if save_q:
        ...
        return statistics,best_itr,env.env.show_history()
...
def run_em_sac(variant):
    ...
    setup_logger(variant['run_name'], variant=variant, log_dir=variant['log_dir'],
                 ...
                 script_name=str(variant['kwargs']))
    ...
    obs_dim = env.observation_space.shape  #(num_channels, self.state_hist)也就是状态种类×状态长度
...
def run_em_dqn(variant):
    ...
    algorithm = DQN(
        ...
        qf_criterion=variant['loss_function'],
        replay_buffer=replay_buffer,
        action_map=[0, env.ideal_basal, 5 * env.ideal_basal],
        **variant['algo_params']
    )
    algorithm.to(variant['device'])
    print('start train')
...
def finish_sac(variant):
    ...
    # for submod in range(variant['num_eval_runs']):
    #     print('Using {}'.format(variant['finish_itr']))
    #     hist,best_itr,all_data = simulate_policy(variant, itr=variant['finish_itr'], sim_seed_submod=submod, use_min=variant['use_min'])
    #     # hist,best_itr = simulate_policy(variant, itr='187', sim_seed_submod=submod, use_min=variant['use_min'])
    #     print(submod)
    #     if submod == 0:
    #         # some backwards compatibility
    #         result_data=pd.DataFrame.from_dict(hist)
    #         print(result_data)
    #         # joblib.dump(hist, '{}/simulate_best{}.pkl'.format(variant['log_dir'], suffix))
    #         joblib.dump(all_data, '{}/simulate_detail{}.pkl'.format(variant['log_dir'], suffix))
    #     else:
    #         # result_data=result_data.append(pd.DataFrame.from_dict(hist))
    #     break
        # result_data.to_csv('{}/simulate_best_{}_{}.csv'.format(variant['log_dir'], suffix,best_itr))
        # joblib.dump(hist, '{}/simulate_best_{}{}.pkl'.format(variant['log_dir'], submod, suffix))
    for submod in range(variant['num_eval_runs']):
        print('Using {}'.format(variant['finish_itr']))
        hist,best_itr,all_data = simulate_policy(variant, itr=variant['finish_itr'], sim_seed_submod=submod, use_min=variant['use_min'])
        ...
        if submod == 0:
            ...
            result_data=pd.DataFrame.from_dict(hist)
            joblib.dump(hist, '{}/simulate_best{}.pkl'.format(variant['log_dir'], suffix))
            # joblib.all_data(hist, '{}/simulate_best{}.pkl'.format(variant['log_dir'], suffix))
        else:
            result_data=result_data.append(pd.DataFrame.from_dict(hist))
        print('{}/simulate_best_{}_{}.csv'.format(variant['log_dir'], suffix,best_itr))
        result_data.to_csv('{}/simulate_best_{}_{}.csv'.format(variant['log_dir'], suffix,best_itr))
        joblib.dump(hist, '{}/simulate_best_{}{}.pkl'.format(variant['log_dir'], submod, suffix))

def run_ts_LSTM(variant):
    # why no logger?
    if variant['independent_init'] is not None:
        torch.manual_seed(variant['independent_init'])
    else:
